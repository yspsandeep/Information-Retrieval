...
index_title_file = os.path.join(current_directory, 'output', 'indexTitle.txt')
index_content_file = os.path.join(current_directory, 'output', 'indexContent.txt')
    ...
    for docid in docid_list:
        ...
        for queryWord in word_list:
            ...
            if valueTitle != 0 & valueContent != 0:
                similarity += gamma * valueTitle * docval * queryval
                totaldocVal = docval
            elif valueTitle != 0:
                docval = getdocValue(queryWord)
                similarity += alpha* valueTitle * docval * queryval
                totaldocVal = docval
            elif valueContent != 0:
                docval = getdocValue(queryWord)
                similarity += beta * valueContent * docval * queryval
                totaldocVal = docval

            if totaldocVal!=0:
                doclist.append(totaldocVal)
        if choice_doc == '3':
